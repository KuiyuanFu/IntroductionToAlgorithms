...
近似的**完全二叉树**，可以使用数组、节点等物理结构。通常使用数组，方便计算。
...
一棵完全二叉树，枝干节点是**输入序列两个位置比较运算操作**，子树是**对应结果的分支**，叶子节点存储结果。
...
右旋：把左子节点旋转到父节点的位置上，**原左子节点的右节点**成为**原父节点的左子节点**，**原父节点**成为**原左子节点的右节点**。

左旋：把右子节点旋转到父节点的位置上，**原右子节点的左节点**成为**原父节点的右子节点**，**原父节点**成为**原右子节点的左节点**。
...
顺序统计树，在红白树的基础上，**每个节点多一个属性**，用来标记包括此节点在内的子树上所有的节点个数。
...
给定一个已排序序列，每个元素都有一个查询概率，还有 $n+1$个 伪元素，表示无值，放在叶子节点位置上。设计二叉树，使其**搜索时间期望最小**。



- 结构：如果此树是最优二叉搜索树，那么其左右子树都是最优二叉搜索树。
- 递归算法：对于一个序列，可以从任意位置分为三部分，一个元素，左右两个子树。
- 计算代价：搜索**元素代价**加**左右子树代价**加**左右子树概率**，左右子树概率的含义为树节点深度加1，代价额外增多 **左右子树概率*1**。需要保存分割位置，总访问概率，代价期望。
- 构造：递归构造
...
**编码树**：构造前缀码。叶子节点上为字符，左为0，右为1，前缀就是唯一的。

**赫尔曼夫编码**：一种**前缀码**，使用贪心算法构造**编码树**，进而构造**前缀码**。使用递归思想，**自底向上**构造，每一步选择两个概率最小的节点，构造一个节点，**此节点的概率为子节点概率和**，使用**队列**，深度优先构造二叉树。
...
B 树的节点可以有很多孩子，其**大小通常与磁盘单元相同**，以减少IO 操作。
...
B 树，关键字元素和所有卫星数据都**存储在节点上**。

每个节点有**标志位**指示其**是否为叶子节点**。

节点若有 $n $ 个关键字， 则会有 $n+1$ 个子节点指针。

叶子节点具有**相同的深度**，即树的高度 $h$ 。

每个节点包含的关键字**个数有上界和下界**。**最小度数** $t$ ，元素个数最少为 $t-1$ 个，最多为 $2t-1$个。

高度与最小度数的关系：$h \leq  \log _t \frac{n+1}2$ ，因为根节点至少有一个子节点，而其他节点都至少有 $t-1$ 个，累加起来，等比数列求和。



B+ 树，所有的卫星数据都存储在**叶子节点**上。
...
- 搜索：树的搜索推广，$O(t)$ 。
- 创建：$O(1)$。
- 插入：如果一个节点满了，那么就把这个**节点分裂成两个节点**，插入到父节点中，如果父节点也满了，继续分裂。实际中，是**从顶至下**操作，每次遇到满节点，就分裂这个节点，保证了要插入的节点不是满的，也就不用向上回溯了。
- 删除：关键字 $k$
  - 是叶子节点，直接删除
  - 是内部节点 $x$，
    - 找到位于 $k$ 前的子节点 $y$ ，如果 $y$ 的子节点个数大于等于 $t$ ，则在 $y$ 中找 $k$ 的**前驱** $k'$，替换，且递归删除 $k’$ 。
    - 否则，找位于 $k$ 后的子节点 $z$ ，如果 $z$ 的子节点个数大于等于 $t$ ，则在 $z$ 中找 $k$ 的**后继** $k'$，替换，且递归删除 $k’$ 。
    - 否则，合并 $y$ 与 $z$ ，删除 $k$ 后，移动 $x$ 的子节点指针。不用替换了。如果 $x$ 此时子节点数小于 $t$ ，需要其的兄弟节点向其移动一个子节点，如果都不够，就**合并**。**递归**。
    - 如果是根节点，且只有一个节点，就会降低树的高度。
...
势函数： $\Phi(H) = t(h)+2m(H)$，$m(H)$ 为标记的节点数。
...
把最小值的**所有孩子放到根链表**中，删去最小值，之后更新根链表，使根链表中，**每一个度的节点至多只有一个**，以减少节点。



具体做法是，使用一个大小为最大度数的数组，依次根据节点的度存储节点指针，一旦**一个度已有指针**，就**合并**这两个节点。做法就是以值较小的节点作为父节点，而**另一个节点作为其的子节点**，这样父节点的度就增加了，再判断这个度是否已经有节点。
...
将节点的关键字替换为指定的小于其值的值，之后判断其**新值是否大于其父节点**，若大于则对其进行切断操作，将其插入到根链表中，之后对父节点判断标志位，**若是假则标记**；若是真，则标记为假，并切断，插入到根链表中，**递归父节点**。



删除操作就是把节点的**值**设置为**负无穷**，之后**删掉**树中的**最小值**。
...
对于每一个节点，其度最多是插入树时的值减一，因为第二次失去子节点就又回到根链表中了。

$F(k) = F(k-1) + F(k-2)$

$F(k+2) = 1 + \sum _{i=0} ^k F(k)$

为什么叫斐波那契堆，因为在处理根链表，使其每个度最多有一个节点时，也使**所有节点的孩子的度是像斐波那契排列的，依次递减。**
...
- **叠加的二叉树结构**：在**直接寻址的数组上叠加一个二叉树**，二叉树的叶子节点就是数组元素，每一个节点有一个标志位，**标记是否**其子树叶子节点有**存储**的关键字。此时上述四个操作的最坏代价是 $\Theta(\lg u)$。
- **叠加一棵高度恒定的树**：若叠加一棵度为 $\sqrt{u}$ 的树，那么高度就为2，是个确定值，每个节点具有 $\sqrt{u}$ 个子节点，称为一个簇。这样每次进行操作**最多需要访问三个节点，每个节点 $\sqrt{u}$ 代价**，所以此时上述四个操作的最坏代价是 $\sqrt{u}$，虽然比二叉树要差，但是其是 van Emde Boas 树的**重要思想来源**。
...
使用二叉树森林，**每个节点只存储父节点的指针**，找动态集没有链表快，但是**渐进最优**。

改进**合并操作**运行时间的**启发式策略**：

- **按秩合并**：较少节点的树的根节点**指向**较多的树的根节点
- **路径压缩**：节点直接指向根节点。
...
对每一个节点的秩都是小于父节点的。从任何一个节点到根节点的路径上，秩都是严格递增的。秩最多是 $n-1$ 。
...
- **邻接链表的组合**：一个邻接链表的头是起始节点，其他元素都是其直接相连的节点。。
- **邻接矩阵**：一个二维数组，相连置为1。
...
给定**源节点**，得到所有其能到达的节点；再将这些节点视为源节点，得到下一层能到达的所有节点，删掉已经访问过的节点。可以使用不同颜色，区分是否访问过。
...
给定**源节点**，访问其任意一个能到达的节点，将这个节点视为源节点，**递归**，直到节点没有能到达的节点或所有能到达的节点都已访问过，此时**回溯**到上一个节点，访问任意其他能到达的节点。

可以使用**栈**来实现。

根据访问关系，可以构建一个**前驱子图**，**深度优先树**。

可以同时指定多个源节点，此时多棵**深度优先树**会构成**深度优先森林**。

节点的**发现时间**和**完成时间**具有所谓的**括号化结构**（parenthesis structure），即包含所有后继节点的发现时间和完成时间，且是规整的。

两个节点的访问区间，要么是**完全分离**的，要么是**完全包含**的。
...
- **树边**：访问路径上的边。
- **后向边**：边的第一个节点访问顺序在第二个节点**之后**。
- **前向边**：边的第一个节点访问顺序在第二个节点**之前**。
- **横向边**：其他的所有边，如连接两棵树的边，或同一棵树上的两个分支，即两个节点没有访问顺序上的关系。

无向图中，每条边，要么是**树边**，要么是**后向边**。深度优先，所以遇到边，不访问另一个节点的条件就是已经访问过了，所以这个边就是后向边。



### 拓扑排序

拓扑排序是图中所有节点的一种**线性次序**，满足所有边的第一个节点排在第二个节点前。

若有环路，则不存在拓扑排序。

拓扑排序将图上的所有节点在一条水平线上排开。

简单的算法为：**使用深度优先遍历，将生成的深度优先森林倒序排列。**这样就可以保证条件了，因为后生成的深度优先树的所有节点都不可能是之前树的节点的后继，但反之是可能的。
...
图的一个强连通分量，是一个**最大节点集合**，满足集合内任意两个节点是可以**相互抵达**的。

一个图的强连通分量和其转置图的强连通分量相同。

分量图：将**每个强连通分量收缩成一个节点**，形成的无环有向图。分量图中的边，就是将强连通分量收缩后剩余的边。

**二次深度优先搜索**算法：首先深度优先遍历一次，记录每个节点的结束时间，之后求转置图的深度优先遍历，同一个节点内，**优先访问结束时间最大的节点**，得到的深度优先森林中的每一个树就是一个强连通分量。

思想来源：当对转置图进行深度优先遍历时，由于是转置图，边的两个节点位置互换了，实际上是**原图的逆向遍历**，这样逆向遍历的结果是**能到达其的节点**，而不是正向遍历的其能到的节点。由于**优先访问结束时间最大的节点**，根据**括号化结构**可得，其是第一次遍历时，最后一棵树的**根节点**，满足**其能到达这棵树上所有节点**，但是其他树上的节点不能到达其，所以只需要在这棵树上，**找到能到达其的节点**就可以了。
...
集合 $A$ 为森林，初始状态每一个节点为一个树，每次选择连接不同树的最小权重边，将两棵树合并为一棵。

### Prim 算法

集合 $A$ 为树，每次划分为 $(A,V-A)$ ，选择轻量级边。初始状态为权重最小边构成的树，每次将一个新节点加入到树中。
