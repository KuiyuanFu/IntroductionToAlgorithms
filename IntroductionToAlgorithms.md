...
权重可为负的单源最短路径问题。返回布尔值，明确源结点是否可以达到一个负权重回路。$O(VE)$
...
可以运行 $|V|$ 次单源最短路径算法，来解锁所有结点对的最短路径。

但是这样时间复杂度很高：

- 若**无负值权重**，使用 **Dijkstra 算法**。
  - 若使用**线性数组**实现最小队列，那么每一次时间复杂度 $O(V^3+VE) =O(V^3)$ 。
  - 若使用**二叉堆**实现最小队列，那么每一次时间复杂度 $O(VE\lg V)$ 。
  - 若使用**斐波那契堆**实现最小队列，那么每一次时间复杂度 $O(V\lg V)$ 。
...
是一种动态规划算法，朴素算法 $O(V^4)$ ，重复平方算法 $O(V^3 \lg V)$。

最短路径的**结构**：两点之间路径最多为 $n-1$个点，将路径分解为两部分，最后一部分为只包含**一个边**。这样另一部分就是**起点**到**此边第一个点**的最短路径。

**递归求解**：

上表 $m$ 表示迭代次数，每一次，测试所有结点的**三角不等式**。

$\cases{ l_{i,j}^{(0)}=0   \quad   i=j \\ l_{i,j}^{(0)}=1 \quad   i\neq j}$

$l_{i,j}^{(m)}=\min(l_{i,j}^{(m-1)},\min\limits_{1\leq k\leq n}\{  l_{i,k}^{(m-1)}+w_{ij}  \})=\min\limits_{1\leq k\leq n}\{  l_{i,k}^{(m-1)}+w_{ij}\}$
...
$l_{i,j}^{(m-1)} \to a , w \to b , l_{i,j}^{(m)}\to c, \min \to + $ ，则可以将递归式或为矩阵乘法，$C=AB$。

这样就可以利用矩阵乘法的思想，求高阶乘法时，可以使用重复平方技术，这里也可以使用。

每次**都是使用最新的路径权重**，一直重复计算，直到阶高于矩阵的大小。$O(V^3\lg V)$

### Floyd-Warshall 算法

假设无负环。

上一个算法，是将一个最短路径分成两部分，且后一部分只有一条边。

此算法同样是分成两部分，但后一部分和前一部分**都是一个最短路径**。

**思想来源**：对于任意一个结点，如果其为一条最短路径的中间结点，那么其会连接路径上的左右结点，这三个结点会组成一个最短路径。

所以每次指定一个结点 $k$，**计算以其为跳转结点的三角不等式**，计算最短路径。$d_{ij}^{k} = \min\{d_{ij}^{k-1},d_{ik}^{k-1}+d_{ik}^{k-1}\}$ 。这样就会得到所有结点**均包含在**结点集合 $1,2,\cdots ,k$ 中的**最短路径**。因为若包含 $k$ 结点，那么就会将最短路径分为 $i\to k,k\to j$ 两部分，且这两段的最短路径权重都在上一步得到了。

**前驱矩阵**：会计算 $n$ 个前驱矩阵，每个矩阵较上个矩阵，会修改在此次中更新的最短路径前驱，$\pi_{ij}^{(k)} = \pi_{kj}^{(k-1)} \quad if \quad \pi_{ij}^{(k-1)}> \pi_{ik}^{(k-1)}+\pi_{kj}^{(k-1)}$，之所以修改后的值为 $\pi_{kj}^{(k-1)}$ ，因为路径为 $i\to k,k \to j$，$\pi_{ij}^{(k)} $ 表示的是，$i \to j$ 路径上，$j$ 的前驱，所以要改变为 $j\to k $ 的前驱。

### Johnson 算法 

稀疏图，邻接链表。

时间复杂度 $O(V^2\lg V+VE)$，优于重复平方法和Floyd算法。

**重新赋予权重**：将每一个权重重新赋值为正值。方法为，添加一个指向所有结点的伪结点，之后计算其到所有结点的距离。将结点的值改为距离值，之后新权重为 $\hat{w}(u,v) = {w}(u,v) + h(u) - h(v)$。由于路径上权重相加，会抵消，不影响最短路径结果。

过程：

- 生成添加伪结点后的图；
- 使用Bellman-Ford 算法，判断新图是否有负环；$O(V^2)$
- 若无负环，则设置从伪结点到每个结点的路径权重，根据此来设置结点值，进而设置边的新权重。$O(E)$
- 对每个结点使用Dijkstra算法，计算路径权重。$O(V^2\lg V+VE)$
